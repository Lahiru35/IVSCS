...
| Signal transfer | Inductive coupling — rotating tyre to fixed chassis |
| Power | Self-powered via piezoelectric energy harvesting |
...
### Problem
The tyre rotates continuously while the wheel hub remains stationary. Direct wired connection is not feasible — wires twist and snap. Physical slip rings wear out within days at 14 rotations/second under automotive load.

### Solution — Inductive Coupling (Wireless)

Two coils transfer power and data wirelessly across the rotation gap — no physical contact, no wear, no wires crossing the rotation boundary. Used by Pirelli Cyber Tyre and Bridgestone IntelliTire.

| Component | Detail |
|-----------|--------|
| Rotating side | PVDF sensor + signal conditioning IC + NFC TX coil (in rim) |
| Stationary side | RX coil on brake calliper → vehicle CAN bus / ECU |
| Protocol | NFC at 13.56 MHz — ISO 15693 |
| Data rate | ~26 kbps (sufficient for 50 ms sensor intervals) |
| Wireless power transfer | 5–50 mW |
| Air gap tolerance | 5–20 mm |
| Industry precedent | Pirelli Cyber Tyre, Continental ContiConnect |

### Implementation Steps
1. Design planar spiral TX coil embedded in rim flange — 10–15 turns, copper on FR4
2. Mount RX coil on stationary brake calliper bracket — matching geometry
3. Signal conditioning IC (charge amplifier) converts PVDF charge to voltage
4. Microcontroller (ATtiny85 / nRF52) on rotating side — samples ADC, transmits via NFC
5. Stationary receiver decodes and forwards to CAN bus / USB / Bluetooth
...
### Solution — Piezoelectric Energy Harvesting (Self-Powered)

The PVDF sensors generate electricity when deformed. A charge accumulation circuit (rectifier + supercapacitor) stores energy from each rotation contact cycle and continuously powers the electronics. **No battery required.**

> 2025 journal publication demonstrated sufficient energy harvest to power a BLE sensor node at highway speeds using this approach.

| Parameter | Value |
|-----------|-------|
| Contact force (per tyre) | ~4,000 N (400 kg vehicle load) |
| PVDF charge per contact | ~30,000 pC |
| Power at 60 km/h | ~2–5 μW average continuous |
| MCU (burst-transmit sleep mode) | ~1–5 μW average |
...
4. Regulated 3.3V → MCU deep sleep; wakes every 50 ms, samples, transmits, sleeps
...
| Estimated cost increase | €3–8 per tyre at volume |
...
| 3 | Signal transfer (rotating → fixed) | 🔴 Critical | Inductive coupling / NFC 13.56 MHz | Solved |
| 4 | Power source | 🟡 High | Piezo energy harvesting — self-powered | Solved |
...
> All three critical challenges are resolved by two technology choices: **PVDF-TrFE polymer film** and **inductive coupling** — both proven in current automotive industry research.

---

## Prototype Roadmap

| Phase | Task | Budget | Output |
|-------|------|--------|--------|
| 1 | PVDF disc in rubber foam — bench test surfaces | Rs. 2,000 | Signal classification via Serial Monitor |
| 2 | Arduino signal conditioning + ML classifier | Rs. 1,500 | Dry/Wet/Mud/Sand/Ice detection |
| 3 | Inductive coil prototype — rotating platform | Rs. 4,000 | Wireless data transfer proof |
| 4 | Energy harvesting circuit — self-powered test | Rs. 3,000 | Battery-free operation |
| 5 | IVSCS dashboard integration — real sensor feed | Rs. 0 | Full system demonstration |
